...
from lib.Helper import nonzeror
...
class Organism(): #Base class for all organisms
	...
	def __init__(self, colour): #Initialisation function, run when the class is initialised
		...
		self.maxspeed = nonzero(17-self.size/2)/20 +0.05
		self.xspeed = binaps(self.maxspeed / 2)
		self.yspeed = binaps(self.maxspeed / 2)
		self.xaccel = 0
		self.yaccel = 0
		...
		self.brain = [self.brain_inputs, self.brain_layer_0, self.brain_layer_1,self.brain_outputs]'''
	...
	def update(self): #Function that performs basic tasks on the organism, such as moving
		'''if self.xspeed > self.maxspeed or self.yspeed > self.maxspeed:
		...
		if self.__class__ == Animal: #An organism loses energy, and loses more if it is larger
			self.energy -= 0.05*self.size
		...
		if self.__class__ == Animal: #Speed must never be zero if the organism is an animal
			self.xspeed = nonzeror(self.xspeed)
			self.yspeed = nonzeror(self.yspeed)
		...
		self.hitbox = pygame.Rect(self.x-self.size, self.y-self.size, self.size*2, self.size*2)
	...
	def mutate(self): #Mutates the organism. This can be thought of as a post-init
		...
		#Mutates the seedrange and poison values if the organism is a plant...
		...
		#Ensures that organisms are always large enough to be drawn correctly
		if self.size < 2:
			self.size = 2
